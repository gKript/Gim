...
void	gim_private_key_obj::write	( void ) {
	...
	if ( tmpfp ) {
		...
		fprintf( tmpfp , "%s \n" , PRE_LINE_KEY );
		fprintf( tmpfp , "%s begin private key -\n" , PRE_LINE_KEY );
		...
		fprintf( tmpfp , "\n" );
		fprintf( tmpfp , "%s end private key -\n" , PRE_LINE_KEY );
		fprintf( tmpfp , "%s\n" , PRE_LINE_KEY );
	...
	if ( gim_conf->GetKeyFLAG( "alias" , "enable" ) == __GIM_YES ) {
		...
		if ( hand != NULL ) {
			...
			if ( ! strcmp( Tok[1] , gim_alias->GetKeySTR( "me" , "nick" ) ) ) {		// if equal
				...
				fclose( hand );
				gim_error->set( GIM_ERROR_CRITICAL , "gim_private_key_obj::read", "The user nick of the private key of this home is not the current user nick." , __GIM_ERROR );
